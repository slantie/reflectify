...
const SuccessModal = () => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-xl p-8 max-w-md w-full mx-4 transform transition-all">
            ...
            <h3 className="text-2xl font-bold text-center text-secondary-darker mb-4">
                Feedback Submitted Successfully!
            </h3>
            <p className="text-center text-secondary-dark mb-6">
...
export default function FeedbackForm() {
    ...
    const fetchForm = useCallback(async () => {
        try {
            const response = await fetch(
                `${FEEDBACK_FORM_ACCESS}/${params.token}`
            );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(
                `${FEEDBACK_FORM_SUBMIT}/${params.token}`,
                {
                    method: "POST",
                    headers: {
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-primary-lighter to-secondary-lighter flex items-center justify-center">
                <div className="bg-white rounded-xl shadow-lg p-8 max-w-md w-full">
                    <h2 className="text-2xl font-bold text-secondary-darker mb-6 text-center">
                        Select Your Batch
                    </h2>
                    <div className="grid grid-cols-3 gap-4">
                        {[1, 2, 3].map((batch) => (
                            <button
                                key={batch}
                                onClick={() =>
                                    setSelectedBatch(batch.toString())
                                }
                                className="p-4 bg-primary-lighter rounded-lg text-lg font-semibold text-primary-dark 
                hover:bg-primary-lighter transition-all transform hover:scale-105"
                            >
                                Batch {batch}
                            </button>
                        ))}
        ...
        switch (question.type.toUpperCase()) {
            case "RATING":
                return (
                    <div className="bg-secondary-lighter rounded-lg p-6">
                        <div className="flex justify-between mb-2 text-sm text-secondary-dark">
                            <span>Poor</span>
                            <span>Excellent</span>
                        </div>
                    ...
                    transition-all hover:bg-primary-lighter
                    ${
            ...
            case "TEXT":
                return (
                    <div className="mt-4">
                        <textarea
                            className="w-full p-4 border rounded-lg focus:ring-2 focus:ring-primary-dark focus:border-primary-dark"
                            rows={4}
        ...
        return questions.map((question, index) => (
            ...
            >
                <div className="flex items-start mb-6">
                    <span className="text-2xl font-bold text-primary-dark mr-4">
                        {(startIndex + index + 1).toString().padStart(2, "0")}
                    </span>
                    <div>
                        <h2 className="text-xl font-semibold text-secondary-darker mb-2">
                            {question.text}
                            {question.isRequired && (
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-primary-lighter to-secondary-lighter">
            {showSuccessModal && <SuccessModal />}
            <div className="max-w-7xl mx-auto py-12 px-4">
                <div className="bg-white rounded-xl shadow-lg p-8 mb-4">
                    <h1 className="text-3xl font-bold text-center text-secondary-darker mb-2">
                        {form.title}
                    </h1>
                    <p className="text-center text-secondary-dark mb-2">
                        Your feedback helps us improve the quality of education
                    </p>
                    <div className="flex justify-center space-x-4 text-md text-secondary-lighter0">
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    {generalQuestions && generalQuestions.length > 0 && (
                        <div className="space-y-6">
                            <h2 className="text-2xl font-bold text-secondary-darker border-b pb-2">
                                Lecture Feedback
                            </h2>
                    ...
                    {batchQuestions && batchQuestions.length > 0 && (
                        <div className="space-y-6">
                            <h2 className="text-2xl font-bold text-secondary-darker border-b pb-2 mt-8">
                                Lab Feedback
                            </h2>
                    ...
                    <div className="mt-8 flex justify-end">
                        <button
                            type="submit"
                            className="
                px-8 py-4 bg-primary-dark text-white rounded-xl font-semibold
                transform transition-all hover:bg-primary-dark hover:scale-105
